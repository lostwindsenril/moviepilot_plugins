...
class BrushConfigEnhanced:
    ...
    def __init__(self, config: dict, process_site_config=True):
        ...
        # 增强版新增：促销期剩余时间检测配置
        self.enable_promotion_check = config.get("enable_promotion_check", True)
        self.min_promotion_hours = self.__parse_number(config.get("min_promotion_hours", 24))
        self.promotion_check_mode = config.get("promotion_check_mode", "skip")
        self.promotion_timezone_offset = self.__parse_number(config.get("promotion_timezone_offset", 0))
        self.promotion_check_timeout = self.__parse_number(config.get("promotion_check_timeout", 5))

        self.brush_tag = "刷流增强"  # 使用不同的标签避免冲突
        # 站点独立配置
    ...
    def __initialize_site_config(self):
        ...
        try:
            ...
            for config in site_configs:
                ...
                self.group_site_configs[sitename] = BrushConfigEnhanced(config=full_config, process_site_config=False)
    ...
    "site_skip_tips": true,
    // 增强版新增：促销期剩余时间检测配置
    "enable_promotion_check": true,
    "min_promotion_hours": 24,
    "promotion_check_mode": "skip",
    "promotion_timezone_offset": 0
}]"""
        return desc + config

    def get_site_config(self, sitename):
        """
        根据站点名称获取特定的BrushConfigEnhanced实例。如果没有找到站点特定的配置，则返回全局的BrushConfigEnhanced实例。
        """
...
class BrushFlowEnhanced(_PluginBase):
    # region 全局定义

    # 插件名称
    plugin_name = "站点刷流增强版"
    # 插件描述
    plugin_desc = "基于原版刷流插件的增强版本，新增促销期剩余时间检测功能，避免下载即将过期的促销种子。"
    # 插件图标
    plugin_icon = "brush.jpg"
    # 插件版本
    plugin_version = "1.0.0"
    # 插件作者
    plugin_author = "jxxghp,InfinityPacer,Augment"
    # 作者主页
    ...
    def init_plugin(self, config: dict = None):
        ...
        if not config:
            logger.info("站点刷流增强版任务出错，无法获取插件配置")
            return False
        ...
        if not self.__validate_and_fix_config(config=config):
            self._brush_config = BrushConfigEnhanced(config=config)
        ...
        if not brush_config.brushsites:
            logger.info(f"站点刷流增强版定时服务停止，没有配置站点")
        ...
        if not brush_config.downloader:
            ...
            self.__update_config()
            logger.info(f"站点刷流增强版服务停止，没有配置下载器")
            return
        ...
        if brush_config.onlyonce:
            ...
            logger.info(f"站点刷流增强版服务启动，立即运行一次")
            ...
            logger.info(f"站点刷流增强版检查服务启动，立即运行一次")
    ...
    def service_info(self) -> Optional[ServiceInfo]:
        ...
        if not service:
            self.__log_and_notify_error("站点刷流增强版任务出错，获取下载器实例失败，请检查配置")
            return None

        if service.instance.is_inactive():
            self.__log_and_notify_error("站点刷流增强版任务出错，下载器未连接")
            return None
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._task_brush_enable:
            if brush_config.cron:
                ...
                cron = " ".join(values)
                logger.info(f"站点刷流增强版定时服务启动，执行周期 {cron}")
                cron_trigger = CronTrigger.from_crontab(cron)
                services.append({
                    "id": "BrushFlowEnhanced",
                    "name": "站点刷流增强版增强版服务",
                    "trigger": cron_trigger,
                    "func": self.brush
                })
            else:
                logger.info(f"站点刷流增强版定时服务启动，时间间隔 {self._brush_interval} 分钟")
                services.append({
                    "id": "BrushFlowEnhanced",
                    "name": "站点刷流增强版增强版服务",
                    "trigger": "interval",
        ...
        if brush_config.enabled:
            logger.info(f"站点刷流增强版检查定时服务启动，时间间隔 {self._check_interval} 分钟")
            services.append({
                "id": "BrushFlowEnhancedCheck",
                "name": "站点刷流增强版增强版检查服务",
                "trigger": "interval",
                "func": self.check,
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [
            ...
            "site_config": BrushConfigEnhanced.get_demo_site_config()
        }

    def get_page(self) -> List[dict]:
        # 种子明细
        torrents = self.get_data("torrents_enhanced") or {}
    ...
    def brush(self):
        ...
        with lock:
            ...
            torrent_tasks: Dict[str, dict] = self.get_data("torrents_enhanced") or {}
            ...
            for site in site_infos:
                # 如果站点刷流增强版没有正确响应，说明没有通过前置条件，其他站点也不需要继续刷流了
            ...
            # 保存数据
            self.save_data("torrents_enhanced", torrent_tasks)
            # 保存统计数据
            self.save_data("statistic_enhanced", statistic_info)
            logger.info(f"刷流任务执行完成")
    ...
    def __update_and_save_statistic_info(self, torrent_tasks):
        ...
        statistic_info = self.__get_statistic_info()
        archived_tasks = self.get_data("archived_enhanced") or {}
    ...
    def __get_brush_config(self, sitename: str = None) -> BrushConfigEnhanced:
        """
        获取BrushConfigEnhanced
        """
    ...
    def __validate_and_fix_config(self, config: dict = None) -> bool:
        ...
        config_number_attr_to_desc = {
            ...
            "min_promotion_hours": "最小促销剩余时间",
            "promotion_timezone_offset": "促销期时区偏移",
            "promotion_check_timeout": "促销期检测超时时间"
        }
        ...
        for attr, desc in config_number_attr_to_desc.items():
            ...
            if value and not self.__is_number(value):
                self.__log_and_notify_error(f"站点刷流增强版任务出错，{desc}设置错误：{value}")
                config[attr] = None
        ...
        if active_time_range and not self.__is_valid_time_range(time_range=active_time_range):
            self.__log_and_notify_error(f"站点刷流增强版任务出错，开启时间段设置错误：{active_time_range}")
    ...
    def __update_config(self, brush_config: BrushConfigEnhanced = None):
        """
        根据传入的BrushConfigEnhanced实例更新配置
        """
        ...
        config_mapping = {
            ...
            "enable_promotion_check": brush_config.enable_promotion_check,
            "min_promotion_hours": brush_config.min_promotion_hours,
            "promotion_check_mode": brush_config.promotion_check_mode,
            "promotion_timezone_offset": brush_config.promotion_timezone_offset,
            "promotion_check_timeout": brush_config.promotion_check_timeout,
            "_tabs": self._tabs
        }
    ...
    def __log_and_notify_error(self, message):
        ...
        self.systemmessage.put(message, title="站点刷流增强版")
    ...
    def __get_torrents_size(self) -> int:
        ...
        """
        # 读取种子记录
        task_info = self.get_data("torrents_enhanced") or {}
        if not task_info:
            return 0
    ...
    def __auto_archive_tasks(self, torrent_tasks: Dict[str, dict]) -> None:
       ...
       """
        ...
        # 用于存储已删除的数据
        archived_tasks: Dict[str, dict] = self.get_data("archived_enhanced") or {}
        ...
        self.save_data("archived_enhanced", archived_tasks)
    ...
    def __clear_tasks(self):
        ...
        """
        self.save_data("torrents_enhanced", {})
        self.save_data("archived_enhanced", {})
        self.save_data("unmanaged", {})
        self.save_data("statistic_enhanced", {})

    def __get_statistic_info(self) -> Dict[str, int]:
        """
        获取统计数据
        """
        statistic_info = self.get_data("statistic_enhanced") or {
            "count": 0,
            "deleted": 0,
            "uploaded": 0,
            "downloaded": 0,
